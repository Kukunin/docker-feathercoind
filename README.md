...
```
$ docker run --name some-feathercoin kukunin/feathercoind
```

To run a feathercoin container in the background, pass the `-d` option to `docker run`:

```
$ docker run -d --name some-feathercoin kukunin/feathercoind
```
...
```
$ docker create -v /data --name ftcdata busybox chown 999:999 /data
$ docker run --volumes-from ftcdata -d -p 9336:9336 -p 127.0.0.1:9337:9337 kukunin/feathercoind
```
